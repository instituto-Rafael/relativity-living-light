...
class MpembaHorizonFalsifierTests(unittest.TestCase):
    ...
    def test_tolman_temperature_increases_for_static_observer_near_horizon(self):
        ...
        rs = MODULE.schwarzschild_radius_m(mass)
        self.assertGreater(
            MODULE.tolman_local_temperature_k(1.0, mass, 1.01 * rs),
            MODULE.tolman_local_temperature_k(1.0, mass, 10.0 * rs),
        )

    def test_mpemba_witness_requires_crossing_and_faster_first_passage(self):
        result = MODULE.mpemba_witness(
            [0.0, 1.0, 2.0, 3.0, 4.0],
            [1.0, 0.50, 0.18, 0.06, 0.02],
    ...
    def test_uncalibrated_symbolic_entropy_equivalence_is_rejected(self):
        states = {item["id"]: item["state"] for item in MODULE.claim_ledger()}
        self.assertEqual(states["BH-MP-10"], "REJECT_UNCALIBRATED_EQUIVALENCE")
        gate = MODULE.symbolic_bh_bridge_gate(
        ...
        self.assertFalse(gate.eligible_for_physical_equivalence_test)

    def test_symbolic_bridge_requires_every_independent_gate(self):
        gate = MODULE.symbolic_bh_bridge_gate(
            dimensional_map_declared=True,
            area_law_recovered=True,
            schwarzschild_first_law_recovered=True,
            observer_covariance_declared=True,
            no_posthoc_unit_adjustment=True,
            independent_prediction_declared=False,
        )
        self.assertFalse(gate.eligible_for_physical_equivalence_test)
        states = {item["id"]: item["state"] for item in MODULE.claim_ledger()}
        self.assertEqual(
            states["BH-MP-11"],
            "ANALOGY_ONLY_TOKEN_VAZIO_COVARIANT_DYNAMICS",
        )
    ...
    def test_baseline_is_bounded_not_global_claim(self):
        ...
        self.assertFalse(
            result["symbolic_drive_crosswalk"]["physical_equivalence_claim_allowed"]
        )
