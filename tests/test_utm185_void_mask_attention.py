...
class UTM185Tests(unittest.TestCase):
    ...
    def test_masked_position_gets_exact_zero_weight(self):
        result = masked_hyperbolic_attention(
            query=(0.0, 0.0), keys=((0.1, 0.0), (0.2, 0.0)),
            values=((1.0,), (9999.0,)), valid_mask=(True, False),
        )
    ...
    def test_zero_key_is_not_missing(self):
        result = masked_hyperbolic_attention(
            query=(0.0, 0.0), keys=((0.0, 0.0), (0.8, 0.0)),
            values=((7.0,), (11.0,)), valid_mask=(True, True),
        )
    ...
    def test_all_masked_fails_closed(self):
        result = masked_hyperbolic_attention(
            query=(0.0, 0.0), keys=((0.1, 0.0), (0.2, 0.0)),
            values=((1.0, 2.0), (3.0, 4.0)), valid_mask=(False, False),
        )
    ...
    def test_weights_sum_to_one_over_valid_positions(self):
        result = masked_hyperbolic_attention(
            query=(0.1, 0.1), keys=((0.2, 0.1), (0.1, 0.2), (0.0, 0.0)),
            values=((1.0,), (2.0,), (3.0,)), valid_mask=(True, False, True),
        )
    ...
    def test_rejects_shape_mismatch(self):
        with self.assertRaises(ValueError):
            masked_hyperbolic_attention(
                query=(0.0, 0.0), keys=((0.1, 0.0),),
                values=((1.0,), (2.0,)), valid_mask=(True,),
            )
